...
const getEnvVariables = () => {
  ...
  const orderBusURL = `http://${VITE_BACKEND_HOST}:${backendPort}/${VITE_ORDER_BUS_SERVICE_ENDPOINT}`;
  const feedbackURL = `http://${VITE_BACKEND_HOST}:${backendPort}/${VITE_FEEDBACK_SERVICE_ENDPOINT}`;
  const webSocketOrderBusServiceURL = `ws://${VITE_BACKEND_HOST}:${backendPort}/${VITE_ORDER_BUS_SRVICE_WS_ENDPOINT}`;
